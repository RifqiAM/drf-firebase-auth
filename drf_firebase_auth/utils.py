...
def get_firebase_user_email(firebase_user: auth.UserRecord) -> str:
    try:
        return (firebase_user.email if firebase_user.email else
...
def map_firebase_to_username_legacy(firebase_user: auth.UserRecord) -> str:
    try:
        username = '_'.join(
            firebase_user.display_name.split(' ') if firebase_user.
            display_name else str(uuid.uuid4()))
...
def map_firebase_display_name_to_username(
        firebase_user: auth.UserRecord) -> str:
    try:
